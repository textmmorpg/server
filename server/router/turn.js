...
function add_routes(socket, io) {
    ...
    socket.on('turn slight left', function(data) {
        ...
        crud_interact.announce(socket.id, io, 'turned slight left', config.SEEING_DISTANCE, true);
    });

    socket.on('turn left', function(data) {
        crud_move.move(socket, io, 0, Math.PI/3, 'turn', false);
        crud_interact.announce(socket.id, io, 'turned left', config.SEEING_DISTANCE, true);
    });

    socket.on('turn hard left', function(data) {
        crud_move.move(socket, io, 0, Math.PI/2, 'turn', false);
        crud_interact.announce(socket.id, io, 'turned hard left', config.SEEING_DISTANCE, true);
    });

    socket.on('turn a little to the left', function(data) {
        crud_move.move(socket, io, 0, Math.PI/6 * Math.random(), 'turn', false);
        crud_interact.announce(socket.id, io, 'turned a bit to the left', config.SEEING_DISTANCE, true);
    ...
    socket.on('turn slight right', function(data) {
        ...
        crud_interact.announce(socket.id, io, 'turned slight right', config.SEEING_DISTANCE, true);
    });

    socket.on('turn right', function(data) {
        crud_move.move(socket, io, 0, Math.PI/3 * -1, 'turn', false);
        crud_interact.announce(socket.id, io, 'turned right', config.SEEING_DISTANCE, true);
    });

    socket.on('turn hard right', function(data) {
        crud_move.move(socket, io, 0, Math.PI/2 * -1, 'turn', false);
        crud_interact.announce(socket.id, io, 'turned hard right', config.SEEING_DISTANCE, true);
    });

    socket.on('turn around', function(data) {
        crud_move.move(socket, io, 0, Math.PI, 'turn', false);
        crud_interact.announce(socket.id, io, 'turned around', config.SEEING_DISTANCE, true);
    });

    socket.on('turn a little to the right', function(data) {
        crud_move.move(socket, io, 0, Math.PI/6 * Math.random() * -1, 'turn', false);
        crud_interact.announce(socket.id, io, 'turned a bit to the right', config.SEEING_DISTANCE, true);
    });
    ...
    socket.on('turn to face north', function(data) {
        ...
        crud_interact.announce(socket.id, io, 'turned to face north', config.SEEING_DISTANCE, true);
    });

    socket.on('turn to face south', function(data) {
        crud_move.move(socket, io, 0, config.SOUTH, 'turn', true);
        crud_interact.announce(socket.id, io, 'turned to face south', config.SEEING_DISTANCE, true);
    });

    socket.on('turn to face east', function(data) {
        crud_move.move(socket, io, 0, config.EAST, 'turn', true);
        crud_interact.announce(socket.id, io, 'turned to face east', config.SEEING_DISTANCE, true);
    });

    socket.on('turn to face west', function(data) {
        crud_move.move(socket, io, 0, config.WEST, 'turn', true);
        crud_interact.announce(socket.id, io, 'turned to face west', config.SEEING_DISTANCE, true);
    });
}
